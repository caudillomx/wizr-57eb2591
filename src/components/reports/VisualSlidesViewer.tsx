...
import { buildSlidesReport } from "@/lib/reports/slidesReportBuilder";
...
export function VisualSlidesViewer({ open, onOpenChange, report, projectName, dateRange }: Props) {
  ...
  const built = useMemo(
    () => buildSlidesReport(report, projectName, dateRange),
    [report, projectName, dateRange],
  );
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      const filename = `reporte_visual_${projectName.replace(/\s+/g, "_")}_${format(new Date(), "yyyyMMdd_HHmm")}.pdf`;
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-slate-950 border-b border-slate-800 text-white">
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold">Reporte Visual</span>
            <span className="text-xs text-slate-400">
              {projectName} · {dateRange.label}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 px-2">
              {current + 1} / {built.count}
            </span>
            <Button
              size="sm"
              variant="ghost"
